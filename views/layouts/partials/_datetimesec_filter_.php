...
<ul class="nav nav-pills">
    <li role="presentation"class="mr-3"
		...
		?>
    >
        <a href="<?= yii\helpers\Url::base() ?><?= $url ?><?= $new_params ?>daily"
		class="btn btn-primary btn-sm <?php if (!isset($_GET['criterion']) || $_GET['criterion'] == 'daily') { echo 'active'; } ?>"><i
    ...
    ><a marked="1"
        href="<?= yii\helpers\Url::base() ?><?= $url ?><?= $new_params ?>range"><span
                    class="fa fa-calendar-alt"></span> Range</a></li>
</ul>
</div>
<div class="row">
    <br><br>
